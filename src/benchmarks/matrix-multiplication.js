...
(function () {
  ...
  function simdMultiply(n) {
    for (var i = 0; i < n; i++) {
      ...
      var a3 = T1x4.getAt(3);
      var b0 = T2x4.getAt(0);
      Outx4.setAt(0, SIMD.add(
                      SIMD.mul(SIMD.shuffle(b0, SIMD.XXXX), a0),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b0, SIMD.YYYY), a1),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b0, SIMD.ZZZZ), a2),
                      SIMD.mul(SIMD.shuffle(b0, SIMD.WWWW), a3)))))
      var b1 = T2x4.getAt(1);
      Outx4.setAt(1, SIMD.add(
                      SIMD.mul(SIMD.shuffle(b1, SIMD.XXXX), a0),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b1, SIMD.YYYY), a1),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b1, SIMD.ZZZZ), a2),
                      SIMD.mul(SIMD.shuffle(b1, SIMD.WWWW), a3)))))
      var b2 = T2x4.getAt(2);
      Outx4.setAt(2, SIMD.add(
                      SIMD.mul(SIMD.shuffle(b2, SIMD.XXXX), a0),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b2, SIMD.YYYY), a1),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b2, SIMD.ZZZZ), a2),
                      SIMD.mul(SIMD.shuffle(b2, SIMD.WWWW), a3)))))
      var b3 = T2x4.getAt(3);
      Outx4.setAt(3, SIMD.add(
                      SIMD.mul(SIMD.shuffle(b3, SIMD.XXXX), a0),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b3, SIMD.YYYY), a1),
                    SIMD.add(
                      SIMD.mul(SIMD.shuffle(b3, SIMD.ZZZZ), a2),
                      SIMD.mul(SIMD.shuffle(b3, SIMD.WWWW), a3)))))
    }
  }
